...
{
    int i, p = periodo_us[n] / 2, t_cc = 1000 * t_ms / (p * 2); /* NOTE: el 2 es para compensar los ciclos de reloj que conllevan las funciones empleadas. Se podría mirar en el .hex para ver cuántas instrucciones se ejecutan en el bucle para tener un ajuste más preciso... */
